...
function getPublicResolveBaseUrl() {
  return cleanBase(
    process.env.RESOLVE_PUBLIC_BASE_URL ||
      process.env.APP_URL ||
      "https://app.boomnow.com"
  );
}
...
export async function resolveViaPublicEndpoint(idOrSlug) {
  const baseUrl = getPublicResolveBaseUrl();
  if (!baseUrl) return null;
  const id = String(idOrSlug ?? "").trim();
  if (!id) return null;
  const url = `${baseUrl}/api/resolve/any?id=${encodeURIComponent(id)}`;
  try {
    const res = await fetch(url, { method: "GET" });
    if (!res.ok) return null;
    const json = await res.json().catch(() => null);
    const uuid = json?.uuid;
    return typeof uuid === "string" && isUuid(uuid) ? uuid.toLowerCase() : null;
  } catch {
    return null;
  }
}
