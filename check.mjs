 ...
 * using URLÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂsafe Base64 encoding. This helper normalises the input and
 * pads it to a multiple of 4 before decoding. If the decoded string
 * contains nonÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂprintable characters or cannot be decoded it returns
 * null.
 *
 * @param {string} str The candidate string to decode
 * @returns {string|null} Decoded UTFÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ8 string or null if decoding fails
 */
function tryDecode(str) {
  if (!str || typeof str !== "string") return null;
  // Replace URLÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂsafe characters
...
function evaluate(messages, now = new Date(), slaMin = SLA_MINUTES) {
  ...
  // COUNT_AI_AS_AGENT is false) are treated like internal notes ÃÂ¢ÃÂÃÂ
...
(async () => {
  ...
  if (!result.ok && result.reason === "guest_unanswered") {
    ...
    // original code contained incorrectly encoded characters (e.g. "ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ÃÂÃÂÃÂÃÂ¯ÃÂÃÂÃÂÃÂ¸ÃÂÃÂÃÂÃÂ" and
    // "ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¥") which manifested as "ÃÂ" symbols in emails.  Use proper
