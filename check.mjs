...
/**
 * Evaluate whether the most recent guest message has gone unanswered beyond the SLA.
 *
 * The original implementation compared the current time to the last agent
 * message, which incorrectly triggered alerts when a new guest message
 * arrived after a long period of inactivity even if it was only seconds old.
 *
 * This revised logic walks backwards through the message list and looks
 * for the most recent user-facing message. If we encounter an agent
 * response (or an AI suggestion when COUNT_AI_AS_AGENT is true) we stop
 * searching because the last guest message has been answered. If we
 * encounter an unresponded guest message (or an AI suggestion when
 * COUNT_AI_AS_AGENT is false) we compute the elapsed minutes since it was
 * sent and compare that to the SLA window. Only if that gap exceeds the
 * SLA do we consider the conversation in breach.
 *
 * @param {Array<Object>} messages List of raw messages from the API
 * @param {Date} [now=new Date()] The current timestamp for comparison
 * @param {number} [slaMin=SLA_MINUTES] SLA threshold in minutes
 * @returns {Object} Result with ok flag, reason, and minsSinceGuest if breached
 */
function evaluate(messages, now = new Date(), slaMin = SLA_MINUTES) {
  // Filter out note/internal messages up front and decorate with parsed timestamps
  const list = (messages || [])
    .filter(Boolean)
    .filter(m => {
      const moduleVal = (m.module || "").toString().toLowerCase();
  ...
  // Sort chronologically to ensure ordering
  list.sort((a, b) => (a._ts?.getTime() || 0) - (b._ts?.getTime() || 0));

  // Walk backwards from the most recent message to determine if the last
  // user-visible message is awaiting a response. We stop once we hit
  // either an agent message (considered a reply) or an unanswered
  // guest/AI message.
  for (let i = list.length - 1; i >= 0; i--) {
    ...
    if (role === "guest" || role === "ai") {
      ...
      // The guest message hasn't aged past the SLA threshold yet, so no breach
      return { ok: true, reason: "no_breach" };
    }
    // Any other roles (e.g. internal) are ignored and we continue searching
  }
  // If we never hit a guest/agent/AI message, assume no breach
  return { ok: true, reason: "no_breach" };
}
